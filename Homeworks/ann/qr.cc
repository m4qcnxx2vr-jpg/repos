...
// ------------------------------------------------------------
// Constructor: Modified / stabilized Gram–Schmidt QR
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Solve QRx = b  (i.e., Ax=b where A=QR)
// Steps: y = Q^T b, then solve R x = y (back substitution)
// ------------------------------------------------------------
pp::vector qr::solve(pp::vector b) const {
    int n = Q.size1();
    int m = Q.size2();
...
// ------------------------------------------------------------
// det(A) = det(R) for our QR (Q is orthogonal)
// det(R) for triangular R is product of diagonal
// ------------------------------------------------------------
double qr::det() const {
    int m = R.size1();
    if (m != R.size2())
...
// ------------------------------------------------------------
// Inverse using solve:
// A^{-1} columns are solutions to A x = e_j
// ------------------------------------------------------------
